...
//	cl_event eventListL[3];
	...
	*/
	...
	cl_int framesize=width*height; 
	clip = 0.004f;
	clip2 = 0.005f; 
	int clipLimit =clip*tileSize[0] * tileSize[1]; 
	int clipLimit2 = clip2*tileSize[0] * tileSize[1];
...
#pragma warning( disable : 4996 )
	...
	cl_mem hsv_out = clCreateBuffer(context, CL_MEM_READ_WRITE, bufferSize, NULL, &err);
	if (err < 0) {
		perror("Couldn't create inputBuffer object\n");
		exit(1);
	}

	int histSize = tiles[0] * tiles[1] * 256*2; // s/v,两个在一个kernel中处理，两个hist放在一个histlut中。
	...
	/*
	cl_mem histV = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(char)*histSize, NULL, &err);
	if (err < 0) {
		perror("Couldn't create histBuffer object\n");
		exit(1);
	}
	*/
	...
	status = clSetKernelArg(kernelCalLut, 6, sizeof(cl_float), &clipLimit2);
	check_err(status, "calLut kernel arg failed,clip");
	status = clSetKernelArg(kernelCalLut, 7, sizeof(cl_int), &framesize);
	check_err(status, "calLut kernel arg failed,clip");
	//status = clSetKernelArg(kernelCalLut, 8, sizeof(cl_mem), &histV);
	//check_err(status, "calLut kernel arg failed,histv");
	...
	//status = clSetKernelArg(kernelMapLut, 8, sizeof(cl_mem), &histV);
	//check_err(status, "mapLut kernel arg failed,histv");
	...
	status = clEnqueueNDRangeKernel(queue, kernelMapLut, 2, NULL, globalSize3, localSizek2, 0, NULL, &event[2]);
